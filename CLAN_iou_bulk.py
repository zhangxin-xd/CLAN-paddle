...
def compute_mIoU(gt_dir, pred_dir, devkit_dir=''):
    ...
    mIoU_mean = mIoUs
    mIoU_mean[[3,4,5,9,14,16]]=0
    mIoU_mean = np.sum(mIoU_mean)/13
    #print(mIoU_mean)
    #print('===> mIoU: ' + str(round(np.nanmean(mIoUs) * 100, 2)))
    print('===> mIoU-13: ' + str(round(mIoU_mean * 100, 2)))
    return str(round(mIoU_mean * 100, 2))
...
if __name__ == "__main__":
    with open("./mIoU_result/SYS2Cityscapes_mIoU.csv","a+",newline="") as datacsv:
        csvwriter = csv.writer(datacsv,dialect = ("excel"))
        for i in range(39, 50):
            gt_dir = './data/Cityscapes/gtFine/val'
            pred_dir = './result/SYS2Cityscapes_{0:d}'.format(i*2000)
