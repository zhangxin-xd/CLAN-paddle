...
class WeightedBCEWithLogitsLoss(paddle.nn.Layer):
...
class CrossEntropyLoss(nn.Layer):
    ...
    def __init__(self,
        ...
        if weight is None:
            self.weight = paddle.ones(shape=[class_num, 1], dtype='int32')
            self.weight=paddle.to_tensor( self.weight,stop_gradient=False)
        '''
        else:
            if weight.stop_gradient==False:
                self.weight = weight
            else:
                self.weight=paddle.to_tensor(weight, dtype='float32',stop_gradient=False)
        '''
    ...
    def forward(self, logit, label, semantic_weights=None):
        ...
        if self.weight is not None and logit.shape[channel_axis] != self.weight.shape[0]:#len(self.weight):
            raise ValueError(
